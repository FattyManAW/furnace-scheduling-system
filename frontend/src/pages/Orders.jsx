...
export default function Orders() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ plan_no: "", contract_no: "", voltage_kv: "", current_a: "", qty: "", delivery_date: "", notes: "" });
    setModalOpen(true);
  };
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      for (const line of lines) {
        try { orders.push(JSON.parse(line)); } catch { continue; }
      }
      if (orders.length === 0) { setBulkMsg("❌ 無有效 JSON 資料"); return; }
  ...
  const statusBadge = (s) => {
    ...
    return <span className={clsx("px-2 py-0.5 rounded-full text-xs font-semibold", map[s] || "")}>{label[s] || s}</span>;
  };
    ...
    <div className="space-y-5">
      ...
      {bulkOpen && (
        <div className="bg-furnace-card border border-furnace-border rounded-xl p-5">
          ...
          <div className="flex items-center justify-between mt-3">
            <p className="text-xs text-furnace-muted">{bulkMsg || "每行一個 JSON 物件"}</p>
            <button onClick={handleBulkImport} className="px-4 py-2 bg-furnace-blue text-white rounded-lg text-sm hover:bg-furnace-blue/80">執行匯入</button>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 items-center bg-furnace-card border border-furnace-border rounded-xl p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {STATUSES.map(s => <option key={s} value={s}>{s === "all" ? "全部狀態" : s === "pending" ? "待排" : s === "scheduled" ? "已排" : "完成"}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="bg-furnace-card border border-furnace-border rounded-xl overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-furnace-border">
                {["#", "計劃單號", "合約號", "電壓", "電流", "數量", "交期", "狀態", "操作"].map(h => (
                  <th key={h} className="text-left px-4 py-3 text-furnace-muted text-xs font-semibold uppercase">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr key={o.id} className="border-b border-furnace-border/50 hover:bg-furnace-bg/50">
                  <td className="px-4 py-3 text-furnace-muted text-xs">{o.id}</td>
                  <td className="px-4 py-3 font-semibold text-furnace-text">{o.plan_no}</td>
                  <td className="px-4 py-3 text-furnace-muted">{o.contract_no || "—"}</td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full bg-furnace-blue/10 text-furnace-blue text-xs font-semibold">{o.voltage_kv} kV</span></td>
                  <td className="px-4 py-3 text-furnace-muted">{o.current_a} A</td>
                  <td className="px-4 py-3 text-furnace-text">{o.qty} 支</td>
                  <td className="px-4 py-3 text-furnace-muted">{o.delivery_date || "—"}</td>
                  <td className="px-4 py-3">{statusBadge(o.status)}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <button onClick={() => openEdit(o)} className="p-1.5 rounded hover:bg-furnace-border/50 text-furnace-muted hover:text-furnace-blue"><Edit3 className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDelete(o.id)} className="p-1.5 rounded hover:bg-furnace-border/50 text-furnace-muted hover:text-furnace-red"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-furnace-border">
          ...
          <div className="flex gap-1">
            <button disabled={page <= 1} onClick={() => setPage(p => p - 1)} className="p-1.5 rounded hover:bg-furnace-border/50 disabled:opacity-30"><ChevronLeft className="w-4 h-4" /></button>
            {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
              ...
              return (
                <button key={p} onClick={() => setPage(p)} className={clsx("w-7 h-7 rounded text-xs font-semibold", p === page ? "bg-furnace-green text-white" : "hover:bg-furnace-border/50 text-furnace-muted")}>{p}</button>
              );
            })}
            <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)} className="p-1.5 rounded hover:bg-furnace-border/50 disabled:opacity-30"><ChevronRight className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
          ...
          <div className="bg-furnace-card border border-furnace-border rounded-2xl p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key}>
                  <label className="block text-xs text-furnace-muted mb-1">{label}</label>
              ...
              {editing && (
                <div className="col-span-2">
                  <label className="block text-xs text-furnace-muted mb-1">狀態</label>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={handleSave} className="flex-1 py-2.5 bg-furnace-green text-white rounded-lg font-semibold text-sm hover:bg-furnace-green/90 flex items-center justify-center gap-1.5">
                <Save className="w-4 h-4" /> 儲存
              </button>
              <button onClick={() => setModalOpen(false)} className="flex-1 py-2.5 border border-furnace-border rounded-lg text-sm text-furnace-muted hover:text-furnace-text">取消</button>
            </div>
          </div>
        </div>
      )}
