    ...
    <div className="fade-slide-up d3 bg-furnace-card hover-lift border border-furnace-border hover:border-furnace-hover rounded-xl p-5 transition-colors">
      <div className="flex items-start justify-between">
        <div className="flex-1 min-w-0">
          <p className="text-furnace-muted text-xs uppercase tracking-wider mb-2">
            {label}
          </p>
          <p
          ...
          {sub && <p className="text-furnace-muted text-xs mt-1.5">{sub}</p>}
        </div>
        <div
          className={`w-11 h-11 rounded-xl flex items-center justify-center ml-3 flex-shrink-0 ${accentBg[accent] || "bg-furnace-hover"}`}
        >
          <Icon
            className={`w-5 h-5 ${accentText[accent] || "text-furnace-muted"}`}
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-between py-2.5">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-semibold text-furnace-heading tabular-nums min-w-[2rem] text-right">
          {count}
        </span>
      </div>
    </div>
  ...
  if (err)
    return (
      <div className="bg-furnace-red/10 border border-furnace-red/20 text-furnace-red rounded-xl p-5 text-sm">
        ...
        載入失敗：{err}
        <button
          className="ml-3 underline hover:text-furnace-heading"
          onClick={() => {
            setLoading(true);
            setErr(null);
            api
    ...
    <div className="fade-slide-up d1 space-y-6">
      ...
      <div className="fade-slide-up d2 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 reveal">
        ...
        <KpiCard
          icon={Factory}
          label="運作中乾燥罐"
          value={`${data.kilns.active_today}/${data.kilns.total}`}
          sub="今日排程"
          accent="purple"
        />
        <KpiCard
          ...
          sub={`每日上限 ${s.daily_cap}h`}
          accent={hourPct > 80 ? "amber" : "green"}
        />
        <KpiCard
          icon={Warehouse}
          label="模具型號庫"
          value={data.molds.total}
          sub="種規格"
          accent="cyan"
        />
      </div>
      ...
      <div className="fade-slide-up d2 grid grid-cols-1 lg:grid-cols-2 gap-4 reveal">
        ...
        <div className="fade-slide-up d5 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-5">
          ...
          {o.pending_by_contract?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-furnace-border">
              <h3 className="text-xs font-semibold text-furnace-muted mb-2 uppercase tracking-wider">
                合約別（待排）
              </h3>
              <div className="fade-slide-up d1 space-y-1 max-h-[160px] overflow-y-auto">
                {o.pending_by_contract.map((c) => (
                  ...
                  >
                    <span className="text-furnace-muted truncate mr-2">
                      {c.contract}
                    </span>
                    <span className="text-furnace-text font-medium">
                      {c.count}
                    </span>
                  </div>
                ))}
        ...
        <div className="fade-slide-up d6 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-5">
          <h2 className="text-sm font-semibold text-furnace-heading mb-4 flex items-center gap-2">
            ...
            逾期警示
            {overdueList.length > 0 && (
              <span className="text-furnace-red text-xs ml-2">
                （{overdueList.length} 筆）
              </span>
            )}
          </h2>
          {overdueList.length === 0 ? (
            ...
            <div className="fade-slide-up d1 space-y-2 max-h-[370px] overflow-y-auto pr-1">
              {overdueList.slice(0, 30).map((o) => (
                <div
                  key={o.id}
                  className="p-3 bg-furnace-red/5 border border-furnace-red/10 rounded-lg hover:border-furnace-red/25 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-furnace-heading">
                        {o.plan_no}
                      </p>
                      <p className="text-xs text-furnace-muted mt-0.5">
                        {o.contract_no || "—"} · {o.qty} 支
                      </p>
                    </div>
              ...
              {overdueList.length > 30 && (
                <p className="text-xs text-furnace-muted text-center pt-1">
                  ...還有 {overdueList.length - 30} 筆
                </p>
              )}
            </div>
          )}
