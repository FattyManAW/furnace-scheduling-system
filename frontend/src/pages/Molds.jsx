  ...
  return (
    <div className="fade-slide-up d2 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-4 hover:border-furnace-blue/30 transition-colors">
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <span
            className={clsx(
              "text-sm font-bold",
              low ? "text-furnace-red" : "text-furnace-green",
            )}
          >
            {mold.stock_qty}
          </span>
        ...
        <div className="w-full h-2 bg-furnace-border rounded-full overflow-hidden">
          <div
            className={clsx(
              ...
              low ? "bg-furnace-red" : "bg-furnace-green",
            )}
            style={{ width: `${pct}%` }}
          />
        </div>
      ...
      <div className="flex gap-1">
        <button
          onClick={() => onAdjust(mold.id, 1)}
          className="flex-1 py-1.5 rounded-lg bg-furnace-green/10 text-furnace-green text-xs font-semibold hover:bg-furnace-green/20 flex items-center justify-center gap-1"
        >
          <Plus className="w-3 h-3" /> 入庫
        </button>
        <button
          onClick={() => onAdjust(mold.id, -1)}
          className="flex-1 py-1.5 rounded-lg bg-furnace-amber/10 text-furnace-amber text-xs font-semibold hover:bg-furnace-amber/20 flex items-center justify-center gap-1"
        >
          <Minus className="w-3 h-3" /> 出庫
        </button>
        <button
          onClick={() => onEdit(mold)}
          className="py-1.5 px-3 rounded-lg border border-furnace-border text-furnace-muted hover:text-furnace-blue text-xs"
        >
          <Edit3 className="w-3 h-3" />
        </button>
      </div>
      {mold.location && (
        <p className="text-[10px] text-furnace-muted mt-2">
          📍 {mold.location}
        </p>
      )}
    </div>
...
export default function Molds() {
  ...
  const openEdit = (m) => {
    ...
    setForm({
      mold_no: m.mold_no,
      outer_dia: m.outer_dia,
      inner_dia: m.inner_dia,
      length: m.length,
  ...
  const filtered = molds.filter(
    (m) =>
      !search ||
      m.mold_no.includes(search) ||
    ...
    <div className="fade-slide-up d1 space-y-5">
      ...
      <div className="fade-slide-up d2 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="fade-slide-up d3 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-furnace-blue">
            {molds.length} 種
          </p>
        </div>
        <div className="fade-slide-up d4 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-4">
          ...
          <p className="text-xl font-bold text-furnace-green">
            {totalStock} 支
          </p>
        </div>
        <div className="fade-slide-up d5 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-4">
          ...
          <p
            className={`text-xl font-bold ${lowCount > 0 ? "text-furnace-red" : "text-furnace-muted"}`}
          >
            {lowCount} 種
          </p>
        </div>
      ...
      <div className="fade-slide-up d6 flex gap-3 bg-furnace-card hover-lift border border-furnace-border rounded-xl p-4">
        ...
        <button
          ...
          className={clsx(
            ...
            lowOnly
              ? "bg-furnace-red/10 border-furnace-red/30 text-furnace-red"
              : "border-furnace-border text-furnace-muted hover:text-furnace-text",
          )}
        >
          僅低庫存
        </button>
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3 mt-5">
              <button
                onClick={handleSave}
                className="flex-1 py-2.5 bg-furnace-green text-white rounded-lg font-semibold text-sm hover:bg-furnace-green/90"
              >
                儲存
              </button>
              <button
                onClick={() => setModalOpen(false)}
                className="flex-1 py-2.5 border border-furnace-border rounded-lg text-sm text-furnace-muted"
              >
                取消
              </button>
            </div>
