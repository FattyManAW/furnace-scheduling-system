        ...
        <div className="fade-slide-up d2 grid grid-cols-1 md:grid-cols-3 gap-3">
          <button onClick={() => downloadCsv("/api/v1/reports/orders/csv", "orders.csv")}
            ...
            <div className="text-left">
              <p className="text-sm font-semibold text-furnace-text group-hover:text-furnace-green">訂單 CSV</p>
              <p className="text-xs text-furnace-muted">匯出所有訂單資料</p>
            </div>
            <Download className="w-4 h-4 text-furnace-muted ml-auto" />
          </button>

          <button onClick={() => downloadCsv("/api/v1/reports/schedule/csv", "schedule.csv")}
            ...
            <div className="text-left">
              <p className="text-sm font-semibold text-furnace-text group-hover:text-furnace-blue">排程 CSV</p>
              <p className="text-xs text-furnace-muted">匯出當前排程結果</p>
            </div>
            <Download className="w-4 h-4 text-furnace-muted ml-auto" />
          </button>

          <button onClick={() => downloadCsv("/api/v1/reports/orders/json", "orders.json")}
            ...
            <div className="text-left">
              <p className="text-sm font-semibold text-furnace-text group-hover:text-furnace-purple">訂單 JSON</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {o.pending_by_contract?.map(c => (
                <tr key={c.contract} className="border-b border-furnace-border/30">
            ...
            {dashboard.overdue_orders.map(o => (
              <div key={o.id} className="fade-slide-up d5 flex items-center justify-between p-3 bg-furnace-card hover-lift rounded-lg border border-furnace-border">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-furnace-muted">交期: {o.delivery_date}</p>
                </div>
              </div>
            ))}
          </div>
